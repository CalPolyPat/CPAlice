...
//    if(!physSelTask) { Printf("no physSelTask"); return; }
    ...
    // create task
    gROOT->ProcessLine(".L AliAnalysisTaskSanity.cxx+g");
    gROOT->LoadMacro("AliAnalysisTaskSanity.cxx");
    AliAnalysisTaskSE* task = new AliAnalysisTaskSanity(taskname);
    //task->SetDoCutsHistos();
    task->DoCutsHistos=kTRUE;
    ...
    TString outfilename = "sanityhistos.root";
  
    // create containers for input/output
    AliAnalysisDataContainer *cinput = mgr->GetCommonInputContainer();
    AliAnalysisDataContainer *coutput1 = mgr->CreateContainer("sanityhistos", TList::Class(), AliAnalysisManager::kOutputContainer, outfilename);
...
//    plugin->SetCheckCopy(kFALSE);
    ...
    plugin->SetDataPattern("emcal_tofrecpoints/*ESDs.root"); // CHECK LATEST PASS OF DATA SET IN ALIENSH
    //    plugin->SetDataPattern("ESDs/pass2/AOD038/*AliAOD.root"); // CHECK LATEST PASS OF DATA SET IN ALIENSH
    plugin->SetRunPrefix("000");   // real data
    // ...then add run numbers to be considered
    Int_t runlist[1]={195673};  
...
//     plugin->AddRunNumber(183913);
    ...
    plugin->SetAnalysisSource("AliAnalysisTaskSanity.cxx");

    // Declare all libraries (other than the default ones for the framework. These will be
    // loaded by the generated analysis macro. Add all extra files (task .cxx/.h) here.
    plugin->SetAdditionalLibs("AliAnalysisTaskSanity.h AliAnalysisTaskSanity.cxx");
