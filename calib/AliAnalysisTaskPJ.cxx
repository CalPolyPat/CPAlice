...
: AliAnalysisTaskSE(name), fESD(0), fOutputList(0), fHistPt(0), fHistTOF(0),fHistNumTOFTOT(0),fHistNumTOFTDC(0),
  ...
  fHistUnmatchedTOFEnergy(0),fEvtNum(0),fHistNum(0),fHistTOFEMEnergyMatch(0), fHistUnmatchedClusPair(0), fHistVel(0), fHistEMClusDist(0), fHistVelT(0), fHistVelD(0), fHistEMClusMatchedDist(0),fHistMatchedClusNum(0),fHistUnMatchedClusNum(0), fHistTotEMClusDist(0), fHistTotCutEMClusDist(0)
...
ent in TOF", 200, 0, 200);
  ...
  fHistTOFEMEnergyMatch = new TH2F("fHistTOFEMEnergyMatch", "Delta R vs. Delta E for Unmatched Clusters", 100, 0, 5, 100, 0, 3);
  ...
  fHistUnmatchedTOFEnergy = new TH1F("fHistUnmatchedTOFEnergy", "TOF Calculated Energy Distribution for Unmatched Clusters with an $e^-$ Mass Assumption", 1000, 0, 1000);
  ...
  fHistEMClusMatchedDist = new TH2F("fHistEMClusMatchedDist", "The Eta Phi Distribution of Matched EMCal Clusters", 98,  -.7, .7, 122, 80*TMath::Pi()/180, TMath::Pi());
  fHistEMClusMatchedDist->GetXaxis()->SetTitle("Eta");
  fHistEMClusMatchedDist->GetYaxis()->SetTitle("Phi");


  fHistEMClusDist = new TH2F("fHistEMClusDist", "THe Eta Phi Distribution of Unmatched EMCal Clusters", 98, -.7, .7, 122, 80*TMath::Pi()/180, TMath::Pi());
  fHistEMClusDist->GetXaxis()->SetTitle("Eta");
  fHistEMClusDist->GetYaxis()->SetTitle("Phi");

  fHistTotEMClusDist = new TH2F("fHistEMClusDist", "THe Eta Phi Distribution of All EMCal Clusters", 98, -.7, .7, 122, 80*TMath::Pi()/180, TMath::Pi());
  fHistTotEMClusDist->GetXaxis()->SetTitle("Eta");
  fHistTotEMClusDist->GetYaxis()->SetTitle("Phi");

  fHistTotCutEMClusDist = new TH2F("fHistEMClusDist", "THe Eta Phi Distribution of  EMCal Clusters with Less Than Three Towers", 98, -.7, .7, 122, 80*TMath::Pi()/180, TMath::Pi());
  fHistTotCutEMClusDist->GetXaxis()->SetTitle("Eta");
  fHistTotCutEMClusDist->GetYaxis()->SetTitle("Phi");
  ...
  fOutputList->Add(fHistTotEMClusDist);
  fOutputList->Add(fHistTotCutEMClusDist);
...
void AliAnalysisTaskPJ::UserExec(Option_t *) 
{
  ...
  for (Int_t icl = 0; icl < nclus; icl++) {    
   ...
   // if (fEvtNum%2000 == 0 && fHistNum < 100) {fHistEtaPhiCC[fHistNum]->Fill(cphi, ceta);}
    ...
    fHistTotEMClusDist->Fill(ceta, cphi);
    if(clus->GetNCells()<4){ fHistTotCutEMClusDist->Fill(ceta, cphi);}
...
Double_t AliAnalysisTaskPJ::CalcMass(Double_t MassHyp, Double_t time, Double_t eta, Bool_t Histo)
{
  if(Histo){
    ...
    Double_t pho_time = (2.5/(TMath::Sin(2*TMath::ATan(TMath::Exp(-eta)))))/c;
    Double_t dist = ((3.70-2.5)/(TMath::Sin(2*TMath::ATan(TMath::Exp(-eta)))));
    Double_t veloc = (dist/(time-pho_time)/c);
  fHistVel->Fill(veloc);
  if (time>8334.497 && time<340236396){
    Double_t c = 0.000299792458;
