...
void runETag(
         ...
         const Long64_t nentries = 30000, // for local and proof mode, ignored in grid mode. Set to 1234567890 for all events.
         ...
         const char *taskname = "PSHFEtask" // sets name of grid generated macros
         )
{
    printf("Made it here");
    // check run type
...
//    iH->SetCheckStatistics(kTRUE);
    ...
    AliAnalysisDataContainer *coutputMB = mgr->CreateContainer("MB", TList::Class(), AliAnalysisManager::kOutputContainer, outfilename);
    AliAnalysisDataContainer *coutputEMC7 = mgr->CreateContainer("EMC7", TList::Class(), AliAnalysisManager::kOutputContainer, outfilename);
    AliAnalysisDataContainer *coutputEMCEGA = mgr->CreateContainer("EMCEGA", TList::Class(), AliAnalysisManager::kOutputContainer, outfilename);
    AliAnalysisDataContainer *coutputEMCJet = mgr->CreateContainer("EMCJet", TList::Class(), AliAnalysisManager::kOutputContainer, outfilename);
        
    // connect input/output
    mgr->ConnectInput(PSHFEtask, 0, cinput);
    mgr->ConnectOutput(PSHFEtask, 1, coutputMB);
    mgr->ConnectOutput(PSHFEtask, 2, coutputEMC7);
    mgr->ConnectOutput(PSHFEtask, 3, coutputEMCEGA);
    mgr->ConnectOutput(PSHFEtask, 4, coutputEMCJet);
...
//    plugin->SetCheckCopy(kFALSE);
    ...
    plugin->SetDataPattern("pass2/*/AliAOD.root"); // CHECK LATEST PASS OF DATA SET IN ALIENSH
...
//     plugin->AddRunNumber(183913);
    ...
    //plugin->SetAdditionalLibs("AliAnalysisTaskPSHFE.h AliAnalysisTaskPSHFE.cxx");
