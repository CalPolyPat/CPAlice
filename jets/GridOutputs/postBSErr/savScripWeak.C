void savScripWeak(){
    ...
    for(Int_t i=0; i<=MB->LastIndex();i++){
        ...
        if(hist->GetName()[5]=='E'&&hist->GetName()[6]=='M'&&hist->GetName()[7]=='C'){
            ...
            if(hist->GetName()[9]=='T'&&hist->GetName()[10]=='P'&&hist->GetName()[11]=='C'&&hist->GetName()[12]=='T'&&hist->GetName()[13]=='R'){            
                ...
                TF1* totalFit = new TF1("totalFit", "gaus(0)+expo(3)", 0.2, 1.2);
        ...
        gStyle->SetOptFit(2);
        TLatex *   tex = new TLatex(0.6,.86,"#splitline{ALICE Work in Progress}{p-p@13TeV}");
        tex->SetNDC();
        tex->SetTextFont(42);
        tex->Draw();
        c1->Update();
    ...
    sigBot->Draw();
    sigTop->Draw();
    TLatex *   tex = new TLatex(0.6,0.8,"#splitline{ALICE Work in Progress}{p-p@13TeV}");
    tex->SetNDC();
    tex->SetTextFont(42);
    tex->Draw();
    c1->Update();
    ...
    for(Int_t i=0;i<=(ind+Offset+8);i++){
        if(i<=ind+Offset+1){
            ...
            if(Dp->Divide(DpM)){
                ...
                if(i%8+1!=8){
                    Cor->Draw("");
                    TLatex *   tex = new TLatex(0.47,0.8,"#splitline{ALICE Work in Progress}{p-p@13TeV}");
                    tex->SetNDC();
                    tex->SetTextFont(42);
                    tex->Draw();
                    /*tite->Draw();*/
    ...
    TLatex *   tex = new TLatex(0.2,0.3,"#splitline{ALICE Work in Progress}{p-p@13TeV}");
    tex->SetNDC();
    tex->SetTextFont(42);
    tex->Draw();
